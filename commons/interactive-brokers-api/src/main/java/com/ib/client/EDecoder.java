...
class EDecoder implements ObjectInput {
	...
	private void processHistoricalTicksBidAsk() throws IOException {
		...
		for (int i = 0; i < tickCount; i++) {
			...
			Decimal sizeBid = readDecimal(), sizeAsk = readDecimal();
	...
	private void processContractDataMsg() throws IOException {
		...
		if (version >= 5) {
			contract.longName(
					m_serverVersion >= EClient.MIN_SERVER_VER_ENCODE_MSG_ASCII7 ? decodeUnicodeEscapedString(readStr())
							: readStr());
		...
		if (m_serverVersion >= EClient.MIN_SERVER_VER_FUND_DATA_FIELDS
	...
	private void processErrMsgMsg() throws IOException {
		...
		if (version < 2) {
			...
			String errorMsg = m_serverVersion >= EClient.MIN_SERVER_VER_ENCODE_MSG_ASCII7
					? decodeUnicodeEscapedString(readStr())
					: readStr();
	...
	private void processTickOptionComputationMsg() throws IOException {
		...
		m_EWrapper.tickOptionComputation(tickerId, tickType, tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma,
				vega, theta, undPrice);
	}
	...
	private void processCompletedOrderMsg() throws IOException {
		...
		EOrderDecoder eOrderDecoder = new EOrderDecoder(this, contract, order, orderState, Integer.MAX_VALUE,
				m_serverVersion);
	...
	private void readLastTradeDate(ContractDetails contract, boolean isBond) throws IOException {
		...
		if (lastTradeDateOrContractMonth != null) {
			String[] splitted = lastTradeDateOrContractMonth.contains("-") ? lastTradeDateOrContractMonth.split("-")
	...
	public Decimal readDecimal() throws IOException {
		String str = readStr();
		return (str == null || str.isEmpty() || str.equals(String.valueOf(Long.MAX_VALUE))
				|| str.equals(String.valueOf(Integer.MAX_VALUE)) || str.equals(String.valueOf(Double.MAX_VALUE)))
						? Decimal.INVALID
						: Decimal.parse(str);
	}
