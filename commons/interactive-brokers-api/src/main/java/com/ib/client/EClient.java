...
public abstract class EClient {
     ...
     * return with date in YYYYMMDD format.
     */
    public synchronized void reqHeadTimestamp(int tickerId, Contract contract, String whatToShow, int useRTH,
            int formatDate) {
        // not connected?
        if (!isConnected()) {
            notConnected();
    ...
    public synchronized void reqMktDepth(int tickerId, Contract contract, int numRows, boolean isSmartDepth,
    ...
    public synchronized void placeOrder(int id, Contract contract, Order order) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_DURATION && order.duration() != Integer.MAX_VALUE) {
            error(id, EClientErrors.UPDATE_TWS, "  It does not support duration attribute");
            return;
        }

        if (m_serverVersion < MIN_SERVER_VER_POST_TO_ATS && order.postToAts() != Integer.MAX_VALUE) {
            error(id, EClientErrors.UPDATE_TWS, "  It does not support postToAts attribute");
            return;
        }

        if (m_serverVersion < MIN_SERVER_VER_AUTO_CANCEL_PARENT && order.autoCancelParent()) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_PEGBEST_PEGMID_OFFSETS) {
            if (order.minTradeQty() != Integer.MAX_VALUE || order.minCompeteSize() != Integer.MAX_VALUE
                    || order.competeAgainstBestOffset() != Double.MAX_VALUE
                    || order.midOffsetAtWhole() != Double.MAX_VALUE || order.midOffsetAtHalf() != Double.MAX_VALUE) {
                error(id, EClientErrors.UPDATE_TWS,
        ...
        try {
            ...
            if (m_serverVersion >= 19) {
                ...
                double lower = (m_serverVersion == 26 && Util.IsVolOrder(order.orderType())) ? Double.MAX_VALUE
                        : order.stockRangeLower();
                double upper = (m_serverVersion == 26 && Util.IsVolOrder(order.orderType())) ? Double.MAX_VALUE
    ...
    public synchronized void reqExecutions(int reqId, ExecutionFilter filter) {
        ...
        try {
            ...
            if (m_serverVersion >= 9) {
                ...
                // Note that the valid format for m_time is "yyyyMMdd-HH:mm:ss" (UTC) or
    ...
    public synchronized void cancelOrder(int id, OrderCancel orderCancel) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_RFQ_FIELDS) {
            if (!IsEmpty(orderCancel.extOperator()) || !IsEmpty(orderCancel.externalUserId())
                    || orderCancel.manualOrderIndicator() != Integer.MAX_VALUE) {
                error(id, EClientErrors.UPDATE_TWS,
    ...
    public synchronized void replaceFA(int reqId, int faDataType, String xml) {
        ...
        if (m_serverVersion >= MIN_SERVER_VER_FA_PROFILE_DESUPPORT && faDataType == 2) {
            error(reqId, EClientErrors.FA_PROFILE_NOT_SUPPORTED.code(), EClientErrors.FA_PROFILE_NOT_SUPPORTED.msg());
            return;
        }
    ...
    public synchronized void reqHistoricalNews(int requestId, int conId, String providerCodes, String startDateTime,
            String endDateTime, int totalResults, List<TagValue> historicalNewsOptions) {
    ...
    public synchronized void reqTickByTickData(int reqId, Contract contract, String tickType, int numberOfTicks,
            boolean ignoreSize) {
        // not connected?
        if (!isConnected()) {
            notConnected();
    ...
    public synchronized void cancelTickByTickData(int reqId) {
        ...
        try {
            ...
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CANCELTICKBYTICKDATA, e.toString());
            close();
        }
    }
    ...
    public synchronized void reqWshMetaData(int reqId) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_WSHE_CALENDAR) {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support WSHE Calendar API.");
            return;
        }
        ...
        try {
            ...
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_REQ_WSH_META_DATA, e.toString());
            close();
        }
    }
    ...
    public synchronized void cancelWshMetaData(int reqId) {
        ...
        try {
            ...
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CAN_WSH_META_DATA, e.toString());
            close();
        }
    }
    ...
    public synchronized void reqWshEventData(int reqId, WshEventData wshEventData) {
        ...
        if (m_serverVersion < MIN_SERVER_VER_WSH_EVENT_DATA_FILTERS) {
            if (!IsEmpty(wshEventData.filter()) || wshEventData.fillWatchlist() || wshEventData.fillPortfolio()
        ...
        try {
            ...
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_REQ_WSH_EVENT_DATA, e.toString());
            close();
        }
    }
    ...
    public synchronized void cancelWshEventData(int reqId) {
        ...
        try {
            ...
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CAN_WSH_EVENT_DATA, e.toString());
            close();
        }
    }
